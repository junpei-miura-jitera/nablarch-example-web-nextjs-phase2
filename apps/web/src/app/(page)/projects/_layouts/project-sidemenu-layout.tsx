 ...
 *
 * 変換メモ:
 * - JSP の `searchForm.` プレフィクス（Nablarch のフォームバインディング用）は
 *   Next.js では不要なため除去し、フラットな name 属性に変換した。
 * - 顧客検索 API: 元の clientList.js は `/api/clients` を呼び出していたが、
 *   OpenAPI 定義に合わせて `/api/client/find` に変更した。
 *
 ...
 */
export function ProjectSidemenuLayout({
  children,
}: {
  ...
  const formActionPath = pathname.startsWith("/projects/bulk")
    ? "/projects/bulk"
    : "/projects";
                  ...
                  <div className="sideMenu">
                    ...
                    <form ref={formRef} method="GET" action={formActionPath} key={searchParams.toString()}>
                      ...
                      <input
                        ...
                        value={
                          searchParams.get("projectStartDateBegin") ?? ""
                        }
                      />
                      <input
                        type="hidden"
                        ...
                        value={
                          searchParams.get("projectStartDateEnd") ?? ""
                        }
                      />
                      <input
                        type="hidden"
                        ...
                        value={
                          searchParams.get("projectEndDateBegin") ?? ""
                        }
                      />
                      <input
                        type="hidden"
                        ...
                        value={
                          searchParams.get("projectEndDateEnd") ?? ""
                        }
                      />
                      ...
                      <ul>
                        <li>
                          ...
                          <ul>
                            {Object.entries(PROJECT_CLASS).map(
                              ([value, label]) => (
                                <li key={value}>
                                  <div className="checkbox form-check">
                                    <label className="form-check-label form-control-lg">
                                      <input
                                        ...
                                        value={value}
                                        defaultChecked={selectedProjectClasses.includes(
                                          value,
                                        )}
                        ...
                        <li className="mb-3">
                          ...
                          <ul>
                            ...
                            <li className="py-2">
                              <a
                                href={periodUrl("startThisYear")}
                                onClick={(e) => {
                                  e.preventDefault();
                                  router.push(periodUrl("startThisYear"));
                                }}
                              >
                                <span className="text-muted fs-5 ps-3">
                                  今年開始
                                </span>
                              </a>
                            </li>
                            <li className="py-2">
                              <a
                                href={periodUrl("endThisYear")}
                                onClick={(e) => {
                                  e.preventDefault();
                                  router.push(periodUrl("endThisYear"));
                                }}
                              >
                                <span className="text-muted fs-5 ps-3">
                                  今年終了
                            ...
                            <li className="py-2">
                              <a
                                href={periodUrl("endLastYear")}
                                onClick={(e) => {
                                  e.preventDefault();
                                  router.push(periodUrl("endLastYear"));
                                }}
                              >
                                <span className="text-muted fs-5 ps-3">
                                  昨年までに終了
