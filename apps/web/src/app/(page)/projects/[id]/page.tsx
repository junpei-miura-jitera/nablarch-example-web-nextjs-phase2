 ...
 * プロジェクト詳細画面。
 *
 * プロジェクトの全フィールドと収益計算結果を表示する。
 * サイドメニューなしで直接 section を配置する（detail.jsp と同一構造）。
 *
  ...
  return (
    <section>
      ...
      <table className="table">
        <tbody>
          <tr><th className="width-250">プロジェクト名</th><td>{project.projectName}</td></tr>
          <tr><th>プロジェクト種別</th><td>{PROJECT_TYPE[project.projectType as keyof typeof PROJECT_TYPE]}</td></tr>
          <tr><th>プロジェクト分類</th><td>{PROJECT_CLASS[project.projectClass as keyof typeof PROJECT_CLASS]}</td></tr>
          <tr><th>プロジェクトマネージャー</th><td>{project.projectManager ?? ""}</td></tr>
          <tr><th>プロジェクトリーダー</th><td>{project.projectLeader ?? ""}</td></tr>
          <tr><th>顧客名</th><td>{project.clientId ?? ""} {project.clientName ?? ""}</td></tr>
          <tr><th>プロジェクト開始日</th><td>{formatDate(project.projectStartDate)}</td></tr>
          <tr><th>プロジェクト終了日</th><td>{formatDate(project.projectEndDate)}</td></tr>
          <tr><th>備考</th><td>{project.note ?? ""}</td></tr>
          <tr><th>売上高</th><td>{project.sales != null ? `${formatMoney(project.sales)} 千円` : ""}</td></tr>
          <tr><th>売上原価</th><td>{project.costOfGoodsSold != null ? `${formatMoney(project.costOfGoodsSold)} 千円` : ""}</td></tr>
          <tr><th>販管費</th><td>{project.sga != null ? `${formatMoney(project.sga)} 千円` : ""}</td></tr>
          <tr><th>本社配賦</th><td>{project.allocationOfCorpExpenses != null ? `${formatMoney(project.allocationOfCorpExpenses)} 千円` : ""}</td></tr>
