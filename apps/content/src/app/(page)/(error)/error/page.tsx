...
import { LayoutFooter } from '../../projects/_components/layouts/layout-footer'
import { LayoutHeader } from '../../projects/_components/layouts/layout-header'
 ...
 * - `hasDynamicMessage` が指定されたエラー種別は `?message=` クエリで サーバーからの動的メッセージも併せて表示する。 `"before"` =
 *   動的メッセージを静的メッセージの前に表示（userError.jsp 相当） `"after"` = 動的メッセージを静的メッセージの後に表示（optimisticLockError.jsp
 *   相当）
 *
...
/**
 * デフォルトのエラー表示（種別不明時）。 — errorPages/error.jsp L26-27
 */
 ...
 * 汎用エラーページ。
 *
 * クエリパラメータ `type` に応じたエラーメッセージを表示する。 元の Nablarch では複数の JSP に分かれていたエラー画面を1ルートに統合。
 *
 * HTML 構造は原本 JSP に忠実:
 *
 * ```
    ...
    <>
      ...
      <div className="message-area">
        ...
        {hasDynamic === 'before' && dynamicMessage && (
          <ul>
            <li className="message-error">{dynamicMessage}</li>
          </ul>
        )}
